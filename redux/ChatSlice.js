...
export const getChatThunk=createAsyncThunk('/getChat',async({userID},{rejectWithValue})=>{
    try{
        const response=await axios.post('http://192.168.0.196:3000/todo/getChat',{
            userID
        })
...
export const messageThunk=createAsyncThunk('/message',async({userID,message})=>{
    try{
        await axios.post('http://192.168.0.196:3000/todo/chat',{
            chat:message,
            userID
        })
    }catch(error){
...
export const sendPromptThunk=createAsyncThunk('prompt',async({prompt,userID})=>{
    try{
        const response=await axios.post('http://192.168.0.196:3000/todo/chatBot',{
            prompt,
            userID
        })
