...
import NoPosts from '../../../assets/SVGS/NoPosts';
...
const SavedImages = () => {
    ...
    function ImageCard({image,index,time,prompt}){
      ...
      function imageDelete(){
        dispatch(DeleteImageThunk({userID,token,image,time}))
        setLike(false)
      }
      ...
      return(
        <View style={{width:'90%',alignSelf:'center',marginVertical:15,marginTop:marginTop,borderRadius:10}}>
          ...
          <View style={{flexDirection:'row',marginLeft:responsiveWidth(2),marginBottom:responsiveHeight(1),borderRadius:10}}>
            ...
            <View>
              <Text style={{marginLeft:responsiveWidth(3),top:responsiveHeight(0.5),fontSize:responsiveFontSize(1.3),fontWeight:'bold',color:'grey',fontFamily:"ProximaNova"}}>Created on : {time}</Text>
              <Text style={{marginLeft:responsiveWidth(3),top:responsiveHeight(0.5),fontSize:responsiveFontSize(1.3),fontWeight:'bold',color:'grey',fontFamily:"ProximaNova"}}>{'@'+email.slice(0,email.indexOf('@'))}</Text>
            </View>
            </View>
          <View>
            <Text style={{marginLeft:responsiveWidth(2),fontSize:responsiveFontSize(2),marginBottom:responsiveHeight(0.1),color:'#AAACB7',fontFamily:"ProximaNova"}}>{prompt}</Text>
          </View>
          ...
          <View style={{backgroundColor:'#282c34',flexDirection:'row',marginVertical:10,marginHorizontal:responsiveWidth(2)}}>
            ...
            >
              ...
              <Text style={{color:'#AAACB7',fontSize:responsiveFontSize(2),fontFamily:"ProximaNova"}}> {like?"Liked":"like"}</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={shareeee} style={{flexDirection:'row',alignItems:'center',flex:1,justifyContent:'center'}}>
              ...
              <Text style={{color:'#AAACB7',fontSize:responsiveFontSize(2),fontFamily:"ProximaNova"}}> Share</Text>
            </TouchableOpacity>
  ...
  }else if(!loading&&savedImages.length==0){
    return(
      <View style={{flex:1,justifyContent:'center',alignItems:'center',backgroundColor:'#202123'}}>
          <NoPosts/>
          <Text style={{fontFamily:'ProximaNova',fontSize:responsiveFontSize(2.5),color:"#AAACB7",bottom:responsiveHeight(75)}}>You have no saved posts</Text>
      </View>
    )
  }
  else{
