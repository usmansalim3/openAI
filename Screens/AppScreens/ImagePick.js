...
const ImagePick = () => {
      ...
      return(
        <View style={{alignSelf:'center',width:'90%',marginVertical:15,marginTop:marginTop,borderRadius:10}}>
          ...
          <View style={{backgroundColor:'#282c34',flexDirection:'row',marginVertical:10,marginHorizontal:responsiveWidth(2)}}>
            <TouchableOpacity style={{flexDirection:'row',flex:1,justifyContent:'center',alignItems:'center'}} onPress={()=>dispatch(uploadImageThunk({token,imageData:image,prompt,time,userID}))}>
              <AntDesign name="like1" size={22} color="#0088cc" />
              <Text style={{color:'#AAACB7',fontSize:responsiveFontSize(2)}}> Liked</Text>
            </TouchableOpacity>
  ...
  if(loading){
    ...
    return (
      <View style={{justifyContent:'center',alignItems:'center',flex:1,backgroundColor:'#202123'}}>
        <Carousel
